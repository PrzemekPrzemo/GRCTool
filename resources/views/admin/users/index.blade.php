            ...
            <tr class="border-t border-slate-100">
                ...
                <td class="px-3 py-2 text-xs">
                    <div class="flex flex-wrap gap-1 max-w-[220px]">
                        @foreach($u->roles as $r)<span class="px-1.5 py-0.5 bg-slate-100 rounded whitespace-nowrap">{{ $r->name }}</span>@endforeach
                    </div>
                </td>
                <td class="px-3 py-2 text-xs whitespace-nowrap">{{ $u->businessUnit?->code ?? '—' }}</td>
                <td class="px-3 py-2 text-xs whitespace-nowrap">{{ $u->is_external ? $u->external_org : '—' }}</td>
                <td class="px-3 py-2 text-xs whitespace-nowrap">{{ $u->hasMfaEnabled() ? '✓' : '⚠' }}</td>
                <td class="px-3 py-2 text-xs whitespace-nowrap">{{ $u->is_active ? 'Tak' : 'Nie' }}</td>
                <td class="px-3 py-2 text-xs whitespace-nowrap">{{ $u->last_login_at?->format('Y-m-d H:i') ?? '—' }}</td>
