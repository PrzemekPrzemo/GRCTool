...
<aside class="fixed inset-y-0 left-0 z-30 w-64 bg-slate-900 text-slate-100 flex flex-col transition-transform duration-200"
    ...
    <nav class="flex-1 overflow-y-auto py-3 text-sm scrollbar-thin">
        ...
        {{-- Risk & Controls --}}
        @canany(['risk.view','control.view','indicator.view'])
        ...
        @canany(['asset.view','vulnerability.view','certificate.view','crypto_key.view','evidence.view'])
        <div class="px-3 mb-1" x-data="{ open: {{ request()->routeIs('assets.*','vulnerabilities.*','certificates.*','crypto-keys.*','evidence.*') ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-transition class="space-y-0.5">
                ...
                <a href="{{ route('certificates.index') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-lg transition-colors {{ request()->routeIs('certificates.*') ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                ...
                @endcan
                @can('crypto_key.view')
                <a href="{{ route('crypto-keys.index') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-lg transition-colors {{ request()->routeIs('crypto-keys.*') ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
                    <span>Klucze kryptograficzne</span>
                </a>
                @endcan
                @can('evidence.view')
        ...
        {{-- Audyty --}}
        @canany(['audit_engagement.view','finding.view','cap.view'])
        ...
        {{-- Klienci & Dostawcy --}}
        @canany(['rfp.view','third_party.view'])
...
<div class="flex flex-col min-h-screen transition-[padding] duration-200" :class="deskOpen ? 'lg:pl-64' : 'lg:pl-0'">
    ...
    <header class="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-slate-200 px-4 lg:px-6 py-3 flex items-center gap-4">
        ...
        <div class="flex items-center gap-2 text-sm">
            ...
            @endcan
            @auth
            @php $alertTotal = collect($systemAlerts)->sum('count'); @endphp
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" title="Alerty" class="relative flex items-center justify-center w-8 h-8 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-colors">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0"/></svg>
                    @if($alertTotal > 0)
                    <span class="absolute -top-0.5 -right-0.5 flex items-center justify-center min-w-[16px] h-4 px-1 rounded-full bg-red-600 text-white text-[10px] font-bold leading-none">{{ $alertTotal > 99 ? '99+' : $alertTotal }}</span>
                    @endif
                </button>
                <div x-show="open" @click.outside="open=false" x-transition
                     class="absolute right-0 mt-2 w-96 max-w-[90vw] max-h-[28rem] overflow-y-auto bg-white border border-slate-200 rounded-lg shadow-lg z-30 text-sm">
                    <div class="px-4 py-2.5 border-b border-slate-100 font-semibold text-slate-700 sticky top-0 bg-white">Alerty @if($alertTotal > 0)({{ $alertTotal }})@endif</div>
                    @forelse($systemAlerts as $group)
                    <div class="px-4 py-2.5 border-b border-slate-50 last:border-0">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-xs font-semibold {{ $group['severity'] === 'red' ? 'text-red-700' : 'text-amber-700' }}">{{ $group['label'] }}</span>
                            <span class="text-xs px-1.5 py-0.5 rounded-full {{ $group['severity'] === 'red' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700' }}">{{ $group['count'] }}</span>
                        </div>
                        <ul class="space-y-1">
                            @foreach($group['items'] as $item)
                            <li><a href="{{ $item['url'] }}" class="block hover:bg-slate-50 rounded px-1.5 py-1 -mx-1.5">
                                <div class="text-xs text-slate-700 truncate">{{ $item['title'] }}</div>
                                <div class="text-[11px] text-slate-400">{{ $item['detail'] }}</div>
                            </a></li>
                            @endforeach
                        </ul>
                        @if($group['count'] > count($group['items']))
                        <a href="{{ $group['viewAllUrl'] }}" class="text-xs text-emerald-600 hover:underline mt-1 inline-block">Zobacz wszystkie ({{ $group['count'] }}) →</a>
                        @endif
                    </div>
                    @empty
                    <div class="px-4 py-6 text-center text-slate-400 text-xs">Brak alertów — wszystko na czas ✓</div>
                    @endforelse
                </div>
            </div>
            @endauth
