...
$alerts = [];
if($stats['incidents_open_p1_p2'] > 0 && $can('incident.view'))   $alerts[] = ['Aktywne incydenty P1/P2: '.$stats['incidents_open_p1_p2'], 'red', route('incidents.index')];
if($stats['gdpr_breach_overdue'] > 0 && $can('gdpr_breach.view'))     $alerts[] = ['Naruszenia RODO po terminie: '.$stats['gdpr_breach_overdue'], 'red', route('gdpr-breaches.index')];
...
$tiles = array_filter([
    ...
    ['Incydenty P1/P2', $stats['incidents_open_p1_p2'], 'otwarte', $stats['incidents_open_p1_p2'] > 0 ? 'red' : 'slate', route('incidents.index'), 'incident.view',
     '<path stroke-linecap="round" stroke-linejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>'],
    ['Szkolenia', $stats['trainings_completion_pct'].'%', 'ukończonych', $stats['trainings_completion_pct'] >= 80 ? 'emerald' : ($stats['trainings_completion_pct'] >= 60 ? 'amber' : 'red'), route('trainings.index'), 'training.view',
     '<path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>'],
], fn ($t) => $t[5] === null || $can($t[5]));
@endphp
@foreach($tiles as [$label, $value, $sub, $color, $href, $perm, $icon])
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 lg:col-span-2">
        ...
        @empty
        <p class="text-sm text-slate-400 py-4 text-center">Brak ryzyk. @can('risk.create')<a href="{{ route('risks.create') }}" class="text-emerald-600 hover:underline">Dodaj pierwsze</a>@endcan</p>
        @endforelse
        </div>
    </div>
</div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <div class="flex items-center justify-between mb-3">
                ...
                @can('compliance.view')<a href="{{ route('compliance.index') }}" class="text-xs text-emerald-600 hover:underline">→</a>@endcan
            </div>
